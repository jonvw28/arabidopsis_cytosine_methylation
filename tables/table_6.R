...
meth.breaks <- stats::quantile(clear.data[,1],
                                            probs = seq(0,1,
                                                        length.out = (classes +1)))
meth.count <- table(clear.data[,ncol(clear.data)])
#
#
...
################################################################################
#
test.data <- dplyr::filter(clear.data, set =="test")
result <- classifier.test(test.data, index = c(8,9))
result
